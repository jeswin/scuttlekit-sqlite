...
The newly generated __id is returned by the insert() function.
...
If __id is missing, the statement will not execute.

```js
async function setCompleted(__id) {
  const db = sdk.getService("sqlite");
...
```js
async function deleteTodo(__id) {
  const db = sdk.getService("sqlite");
  return await db.del("todo", __id);
}
```
